...
char *GetImageLinksFromWebPage(char *myhtmlpage, char *myUrl){
    ...
    /*Extract the links*/
    char * result = extractAttributesOfTag(document, TAG_IMG, attribute);
...
{   
    ...
    size_t urlLen = strlen(argv[1]);
    char *url = (char *)malloc(URL_BUFFER_SIZE*sizeof(char));
    strcpy(url, argv[1]);
    url[urlLen] = '\0';
    ...
    char * webLinks = GetLinksFromWebPage(webpage, url);
    /*Extract image links from webpage*/
    char * imageLinks = GetImageLinksFromWebPage(webpage, url);
    /*Print everything*/
    printf("LINKS:\n%s\n", webLinks);
    printf("IMAGES:\n%s\n", imageLinks);
    // haut_destroy(&parser);
    ...
    if(imageLinks!=NULL) free(imageLinks);
    free(url);
    /*** Added for solution ***/
    return 0;
}
