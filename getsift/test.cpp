...
// printf("Reading images: %s and %s\n",argv[1],argv[2]);
  if(argc != 4) {printf("\n\nUsage: similarimage [image1.jpg] [image2.jpg] [image3.jpg]\n\n"); exit(0);}
	...
	CvPoint pt1, pt2;
	double d0, d1;
	int n1, n2, n3, k, i,j, rmatches1 = 0, rmatches2 = 0, matches1 = 0, matches2 = 0;
	...
	printf("coordinate and descriptor of %s keypoints have been written in featfile2.txt\n",argv[2]);
        ...
        feat3file=fopen("%s.features3.txt","w+");
        for(i=0;i<n3;i++)
        {
	...
	printf("Found %d total matches for %s\n", matches1,argv[2] );
	...
	printf("Found %d total matches after RANSAC for %s\n", rmatches1, argv[2]);
	write_jpeg_file("%s.matches.ransac.jpg",stacked2);
        
   
	// Building matches for img 2
	kd_root = kdtree_build( feat3, n3 );
        ...
        printf("Found %d total matches for %s\n", matches2, argv[3] );
        write_jpeg_file("%s.matches.jpg",stacked3);
        ...
        printf("Found %d total matches after RANSAC for %s\n", rmatches2, argv[3]);
        write_jpeg_file("%s.matches.ransac.jpg",stacked4);
	...
	if (matches1 > matches2) printf("Based on the total amount of matches, %s is more similar to %s than %s\n", argv[2], argv[1], argv[3]);
	else if (matches2 > matches1) printf("Based on the total amount of matches, %s is more similar to %s than %s\n", argv[3], argv[1], argv[2]);
	else printf("Based on the total amount of matches, %s is as similar to %s as  %s\n", argv[2], argv[1], argv[3]);
	return 0;
}
